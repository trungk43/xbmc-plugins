...
SEARCH_URL='http://4share.vn/pub/index/search?page=%s&str=%s'
				...
				for a in soup.findAll('a'):
						try:
							if a['href'].find('/d/')>0:
								...
								if a['href'].find('/d/')==0:
										add_dir(a.text, 'http://4share.vn' + a['href'], 2, icon, '', 'd', page+1)
								else:
									if a['href'].find('/f/')>0:
										if a.findParent('tr').findAll('td')[2].text.find('KB') < 0:
											add_link('', a.text + ' (' + a.findParent('tr').findAll('td')[2].text + ')', 0, a['href'], '', '')
									else:
										if a['href'].find('/f/')==0:
											if a.findParent('tr').findAll('td')[1].text.find('KB') < 0:
												add_link('', a.text + ' (' + a.findParent('tr').findAll('td')[1].text + ')', 0, 'http://4share.vn' + a['href'], '', '')
						except:
								pass			
		except:
				pass			
